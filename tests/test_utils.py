...
import unittest
from small_llama_model import utils
import numpy as np

class TestUtils(unittest.TestCase):
    def test_silu(self):
        input = np.array([0, 0.5, 1, 2, 3, 4])
        output = utils.silu(input)
    ...
    def test_softmax(self):
        ...
        np.testing.assert_array_almost_equal(utils.softmax_last(input), exp)
        k1 = np.e/(4*np.e+4)
        k2 = 1/(4*np.e+4)
        exp2 = np.array([[k1, k1, k1, k1], [k2, k2, k2, k2]])
        np.testing.assert_array_almost_equal(utils.softmax_all(input), exp2)
